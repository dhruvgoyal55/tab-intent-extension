// Intent Popup - Injected into new tabs

(function() {
  // Don't inject multiple times
  if (window.tabIntentPopupInjected) return;
  window.tabIntentPopupInjected = true;
  ...
  document.addEventListener('keydown', (e) => {
    if (e.target !== document.getElementById('intentNote') && 
        e.key.length === 1 && 
        !e.ctrlKey && 
        !e.metaKey) {
      document.getElementById('intentNote').focus();
    }
  });
})();
